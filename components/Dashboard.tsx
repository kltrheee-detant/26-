    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">클럽의 중요 공지 및 월례 라운딩 일정을 확인하세요.</p>
        </div>
        <div className="hidden md:block">
           <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Latest Update: {new Date().toLocaleDateString()}</span>
        </div>
      </header>
           ...
           {upcomingOutings.length > 0 ? upcomingOutings.slice(0, 2).map((outing) => (
             <div key={outing.id} className="bg-white rounded-[2rem] p-6 border-2 border-emerald-50 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                ...
                <div className="mt-5 pt-4 border-t border-slate-50 flex items-center justify-between">
                   ...
                   <span className="text-[10px] font-black text-slate-300 uppercase tracking-tighter">참여 멤버 {outing.participants.length}명</span>
                </div>
             </div>
           )) : (
             <div className="col-span-full bg-white rounded-3xl p-12 text-center border-2 border-dashed border-slate-100">
                <p className="text-slate-400 text-sm font-medium">예정된 월례 라운딩 일정이 없습니다.</p>
                <button onClick={onNavigateOutings} className="mt-3 text-emerald-600 font-bold text-xs hover:underline">일정 등록하러 가기</button>
             </div>
           )}
        </div>
      </section>
